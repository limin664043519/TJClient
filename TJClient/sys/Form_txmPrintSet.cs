...
{
    public partial class Form_txmPrintSet : sysCommonForm
    {
        private static string strWhere = "";
        public Form_txmPrintSet()
        {
            InitializeComponent();
        }
        ...
        {
            Form_txmBll xmfl = new Form_txmBll();
            string strWhere = "";
            if (UserInfo.Yybm != null && UserInfo.Yybm.Trim().Length > 0)
            {
                strWhere = strWhere + string.Format(" and  YLJGBM ='{0}' ", UserInfo.Yybm);
            }

            if (mc.Text.Trim().Length > 0)
            {
                //strWhere = strWhere + string.Format(" and TMMC  like '%{0}%' ", mc.Text.Trim());
            }
            DataTable dt = xmfl.GetMoHuList(strWhere, "sql098");
            showDataGrid(dt);
        }
        ...
        private void button_del_Click(object sender, EventArgs e)
        {
          ...
          try
          {
              DataTable dtupdate = ((DataTable)dataGridView1.DataSource).Copy();
              Form_txmBll SjzdBll = new Form_txmBll();

              if (dtupdate != null && dtupdate.Rows.Count > 0 && dtupdate.Rows.Count > rowIndex)
              {
                  SjzdBll.Del(dtupdate, rowIndex, "sql099");
              }
              dataSelect();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void button_tmset_Click(object sender, EventArgs e)
        {
            //MousePosition.X
            //Point textLocation = PointToScreen(textBox_pym.Location);
            //textLocation.X = textLocation.X + 1;
            //textLocation.Y = textLocation.Y + textBox_pym.Height;
            Form_txmSet form = new Form_txmSet();
            //form.Location = textLocation;
            form.Owner = this;
            form.Show();
            //if (form.setListData(textBox_pym.Text.Trim(), "sql002") == false)
