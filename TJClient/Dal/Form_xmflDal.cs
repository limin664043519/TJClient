...
namespace TJClient.sys.Dal
{
    public class Form_xmflDal
    {
        ...
        {
            string sql_Sjzd = Common.Common.getSql("sql010", strWhere);
            try
            {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            sql_Sjzd = Common.Common.getSql("sql011", dt, i);
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            sql_Sjzd = Common.Common.getSql("sql012", dt, i);
                            sqlList.Add(sql_Sjzd);
                        }
                    }
                }
        ...
        public bool Del(DataTable dt,int rowIndex)
        {
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    sql_Sjzd = Common.Common.getSql("sql013", dt, rowIndex);
