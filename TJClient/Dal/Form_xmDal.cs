...
namespace TJClient.sys.Dal
{
    public class Form_xmDal
    {
        ...
        {
            string sql_Sjzd = Common.Common.getSql("sql014", strWhere);
            try
            {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            sql_Sjzd = Common.Common.getSql("sql015", dt, i);
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            sql_Sjzd = Common.Common.getSql("sql016", dt, i);
                            sqlList.Add(sql_Sjzd);
                        }
                    }
                }
        ...
        public bool Del(DataTable dt,int rowIndex)
        {
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    sql_Sjzd = Common.Common.getSql("sql017", dt, rowIndex);
